...
quizapp.controller("bot3", function($scope, $state, $http, $document){
    ...
    $scope.send_topic_req = () => {
        send_topic_request($scope.req_topic)
        $scope.req_topic = undefined
    }

    $scope.sendFeedback = () => {
        let tst = send_feedback($scope.subject, $scope.body)
        if (tst == true){
...
quizapp.controller("bot4", function($scope, $state, $http, $document){
    $document.ready(function() {
...
quizapp.controller("bot6", function($scope, $http, $state, $document){
    $document.ready(function() {
        var elems = document.querySelectorAll('.datepicker');
        var instances = M.Datepicker.init(elems, {
            format: 'mmmm d, yyyy',
...
quizapp.controller("bot7", function($scope, $state, $http, $document){
    $document.ready(function() {
        var elems = document.querySelectorAll('.datepicker');
        var instances = M.Datepicker.init(elems, {
            format: 'mmmm d, yyyy',
...
quizapp.controller("bot12", function($scope, $state, $http){
    if(user.length==0){
        $state.go("adminlogin")
    }
    else if (user[0].admin==undefined){
        ...
        $http({
            method:"GET",
            url:"https://am-akash1309-quiz-server.glitch.me/get_requested_topics"
        }).then (function(result){
            console.log(result.data)
            $scope.req_topics = result.data
        })
    }
})

quizapp.controller("bot13", function($scope, $state, $http){
    if(user.length==0){
        $state.go("adminlogin")
    }
    else if (user[0].admin==undefined){
        ...
        $http({
            method:"GET",
            url:"https://am-akash1309-quiz-server.glitch.me/get_feedback"
        }).then (function(result){
...
function send_topic_request (topic) {
    var $http = angular.injector(['ng']).get('$http')
    if (topic==undefined){
        M.toast({
            "html": "Please enter the topic",
            "classes": "red rounded"
        })
    }
    else {
    $http({
        method: "POST",
        url:"https://am-akash1309-quiz-server.glitch.me/send_request_topic/",
        data: {
            "name": user[0].name,
            "email": user[0].email,
            "topic": topic
...
function send_feedback (subject, body) {
    var $http = angular.injector(['ng']).get('$http')
    ...
    $http({
        method: "POST",
        url:"https://am-akash1309-quiz-server.glitch.me/send_feedback/",
        data: {
            "name": user[0].name,
            "email": user[0].email,
            "subject": subject,
...
quizapp.config(function($stateProvider, $urlRouterProvider){
    ...
    .state("requested_topics", {
        url: "/Topics/Requested",
        templateUrl:"./requsted_topics.html",
        controller: "bot12"
    })
    .state("feedback", {
        url: "/Feedbacks",
